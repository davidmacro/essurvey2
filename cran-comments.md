...
* GitHub Actions (`r-lib/actions`, `rcmdcheck` with `--as-cran`):
...
All pass with no errors and no warnings. The CI runs set
`_R_CHECK_CRAN_INCOMING_=false`, so the "New submission" note below does not
arise there; it comes from the local `--as-cran` run on the release tarball.
