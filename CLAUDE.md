...
**Tests: skips are expected, failures are not.** The suite is ~249 passing with
**13 skipped**. Every skip is a live-API test that needs `ESS_USER_ID`, which is
not set on this machine. Skips are not a gate failure and are not something to
"fix" — do not try to make them pass or delete them.

**Vignettes: pandoc is unavailable to `Rscript` here**, so
`rmarkdown::render()` fails with a pandoc error on *every* vignette, including
ones that were fine before you touched them. That is the environment, not your
change. Check vignettes this way instead — no pandoc needed, and the parse step
catches syntax errors that `eval = FALSE` chunks would otherwise hide until a
reader ran them:
